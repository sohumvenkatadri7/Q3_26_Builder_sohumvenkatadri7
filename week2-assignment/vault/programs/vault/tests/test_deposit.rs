...
fn test_deposit() {
    ...
    // 2. Assert vault balance increased by deposit amount
    assert_eq!(
        svm.get_balance(&vault).unwrap(),
        rent_exempt + DEPOSIT_LAMPORTS,
        "vault should increase by the deposit amount"
    );
}
...
fn test_deposit_zero_fails() {
    ...
    // Depositing 0 should fail with InvalidAmount
    assert!(svm.send_transaction(tx).is_err(), "deposit with 0 amount should fail");
}
